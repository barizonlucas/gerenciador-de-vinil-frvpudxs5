...
export function RecordModal({ record, isOpen = false, onClose }: RecordModalProps) {
    ...
    <div className="modal">
      <div className="modal-content max-w-4xl p-6">
        <div className="flex justify-between items-start">
          ...
          <button onClick={onClose} aria-label="Fechar" className="text-gray-500">✕</button>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {historyLoading && <div className="p-4 text-center">Carregando história...</div>}
            {historyError && (
              <div className="p-4 text-red-500">{historyError}</div>
            )}
            {!historyLoading && !historyError && history && (
              <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: history }} />
            )}
          </div>
        </div>
